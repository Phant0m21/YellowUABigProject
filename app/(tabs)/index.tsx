...
export default function CatalogScreen() {
  ...
  const filteredProducts = useMemo(() => catalogProducts.filter((product) => {
    ...
    const matchesSearch = !query || `${product.name} ${product.code} ${product.article}`.toLocaleLowerCase().includes(query);
    return matchesSearch && price <= filters.maxPrice && (!filters.memory || product.memory === filters.memory) && (!filters.sim || product.sim === filters.sim) && (!filters.color || product.color === filters.color);
  }), [filters, searchQuery]);
  return <SafeAreaView style={styles.safe}><StatusBar style="light" />
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
      ...
      <View style={styles.content}>
        ...
        <View style={styles.searchBox}><Ionicons name="search-outline" size={20} color="#5e51ff" /><TextInput value={searchQuery} onChangeText={setSearchQuery} placeholder="Пошук за назвою або кодом" placeholderTextColor="#9b9b9b" style={styles.searchInput} autoCorrect={false} autoCapitalize="none" />{searchQuery.length > 0 && <Pressable onPress={() => setSearchQuery('')} hitSlop={10}><Ionicons name="close-circle" size={19} color="#8f8f8f" /></Pressable>}</View>
        ...
        <View style={styles.resultLine}><Text style={styles.resultText}>Знайдено товарів: {filteredProducts.length}</Text><Pressable onPress={() => { setFilters({ memory: null, sim: null, color: null, maxPrice: 80000 }); setSearchQuery(''); }}><Text style={styles.resetText}>Скинути</Text></Pressable></View>
        {filteredProducts.length === 0 && <View style={styles.empty}><Ionicons name="search-outline" size={30} color="#a4a4a4" /><Text style={styles.emptyTitle}>Нічого не знайдено</Text><Text style={styles.emptyText}>Спробуйте змінити запит або скинути фільтри.</Text></View>}
