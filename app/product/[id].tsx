...
import { productImage, products, productSpecs, productSummary } from '@/constants/products';
...
export default function ProductScreen() {
  ...
  return <SafeAreaView style={styles.safe}><YellowHeader /><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    ...
    <View style={styles.meta}><Text style={styles.available}>Є в наявності</Text><View><Text style={styles.article}>Артикул: {product.article}</Text><Text style={styles.article}>Код товару: <Text style={styles.code}>{product.code}</Text></Text></View></View>
    <Text style={styles.name}>{product.name}</Text><Text style={styles.description}>{productSummary(product)}</Text>
    ...
    <Text style={styles.specTitle}>Характеристики</Text><View style={styles.specs}>{specs.map(([label, value]) => <View style={styles.spec} key={label}><Text style={styles.specLabel}>{label}</Text><Text style={styles.specValue}>{value}</Text></View>)}</View>
  </ScrollView></SafeAreaView>;
}
const styles = StyleSheet.create({ safe:{flex:1,backgroundColor:'#1b1b1b'},content:{backgroundColor:'#fff',padding:10,paddingBottom:34,minHeight:'100%'},breadcrumbs:{flexDirection:'row',marginTop:13,gap:8},back:{fontSize:12,color:'#5e5e5e'},crumb:{fontSize:12,color:'#777'},muted:{fontSize:12,color:'#aaa',marginTop:12},image:{height:400,width:'100%',marginTop:14},meta:{flexDirection:'row',justifyContent:'space-between',alignItems:'flex-end'},available:{color:'#20b84b',fontSize:12,backgroundColor:'#f5fff7',paddingHorizontal:10,paddingVertical:4},article:{fontSize:14,lineHeight:25,color:'#aaa',textAlign:'right'},code:{color:'#111'},name:{fontSize:18,lineHeight:23,fontWeight:'600',marginTop:21},description:{fontSize:16,lineHeight:22,marginTop:17},price:{fontSize:31,fontWeight:'700',marginTop:16},currency:{fontSize:14,fontWeight:'400'},actions:{flexDirection:'row',gap:7,marginTop:16},buy:{height:44,backgroundColor:'#ffd000',borderRadius:5,alignItems:'center',justifyContent:'center',flex:1},pressed:{backgroundColor:'#d72668'},buyText:{fontWeight:'600'},compare:{width:44,borderWidth:1,borderColor:'#e5e5e5',borderRadius:6,alignItems:'center',justifyContent:'center'},specTitle:{fontSize:22,fontWeight:'700',marginTop:30,marginBottom:12},specs:{borderTopWidth:1,borderColor:'#e8e8e8'},spec:{paddingVertical:12,borderBottomWidth:1,borderColor:'#e8e8e8'},specLabel:{fontSize:13,color:'#777',marginBottom:3},specValue:{fontSize:15,color:'#111'} });
